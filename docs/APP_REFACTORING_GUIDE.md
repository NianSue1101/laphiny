...
1. `renderSoulRelationsPanel` → `src/components/SoulRelationsPanel.tsx`
2. `renderRoomCollaborationDashboard` → `src/components/RoomCollaborationDashboard.tsx`
3. `RoomGrowthPanel` 内部继续拆：`KnowledgeSection`、`BlackboardSection`、`DecisionRecordsSection`、`RoomSoulRelationsSection`
