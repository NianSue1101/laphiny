...
Soul 房间侧栏抽屉，负责展示房间状态、RP 场景、最近共识、记忆胶囊、成长层摘要、RP 档案、任务看板、委托任务和最近协作事件。

约束：

- 不发起总结、委托或 Hermes 请求；只展示 App 已经计算好的数据。
- 不修改抽屉开关状态；关闭通过 `onClose` 回调完成。
- 使用 `RoomStatusBar` 和 `RoleplaySceneCard` 组合，不在抽屉内复制这两块 UI。
...
### `src/components/RoomCollaborationDashboard.tsx`

聊天页内的 Soul 协作时间线面板，负责展示最近共识、RP 房间摘要、记忆胶囊、成长层摘要、委托任务和最近协作事件。

约束：

- 展开/收起通过 `onToggleOpen` 回调完成。
- 不计算 task/event/growth；只展示传入数据。
- 不触发任何状态写入。

### `src/components/RoomGrowthPanel.tsx`

房间成长层面板，负责展示和编辑房间知识库、协作黑板、决策记录，以及展示本房间 Soul 关系图。

约束：

- 不直接读写 App state；所有新增、删除、状态变更都通过外层回调完成。
- 不计算房间成长摘要；`growth` 由 App 通过 `summarizeRoomGrowth` 计算后传入。
- 不计算 Soul 关系；`soulRelations` 由 App 筛选后传入。
- 后续如果继续拆这个文件，优先拆成 `KnowledgeSection`、`BlackboardSection`、`DecisionRecordsSection`、`RoomSoulRelationsSection`。
...
1. `RoomGrowthPanel` 内部继续拆：`KnowledgeSection`、`BlackboardSection`、`DecisionRecordsSection`、`RoomSoulRelationsSection`
2. `renderQuickCommands` → `src/components/QuickCommandsPanel.tsx`
3. `renderSettings` 内部的设置卡片 → `src/components/settings/*`
4. `renderMessageBubble` → `src/components/chat/MessageBubble.tsx`
5. `StyleSheet.create` → `src/app/app_styles.ts`
