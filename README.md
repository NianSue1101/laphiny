...
- 全量备份 / 合并恢复
- PWA 离线支持
- 可选、自托管的独立 `laphiny-sync` 服务：快照与设置同步、差异预检、事件轮询，以及 Agent 向指定房间主动回复

---

## v0.35.1 更新

- **断线恢复**：支持 Hermes Gateway 明确公布的 Runs API。任务提交后立即持久化 `run_id`，网络中断、应用后台或锁屏后可重新查询同一任务并恢复最终结果，不会重复启动 Agent。
- **旧 Gateway 安全降级**：传统 Chat Completions / Responses SSE 在连接断开时会由 Hermes 中止。Laphiny 现在会识别意外 EOF、远端失败和长度截断，并只在没有工具调用或权限副作用风险时发起一次明确标注的新续作；否则保留部分正文并要求手动确认。
- **移动端错位修复**：修正窄屏、大字体和回复流式更新期间房间选择器横向溢出；离开聊天页时同步失效旧滚动代际，防止排队中的动画帧滚动错误页面。
- **大快照同步**：新版同步后端支持能力协商、UTF-8 安全分片、SHA-256 校验、幂等重传、状态查询、提交回执和 `baseRevision` CAS。小快照仍使用旧接口；旧后端遇到 413 时会给出明确升级提示。
- **恢复状态可同步**：`run_id`、传输类型、恢复次数和 `interrupted` 状态会随快照保存，跨设备合并不会把未完成回复误标为成功。

限制：只有 Gateway 明确支持 Runs API 时才能接回同一服务端任务；旧 SSE 接口不具备真正的断点续传能力，Laphiny 不会推断服务端隐藏状态或自动重放可能产生副作用的工作。

## v0.35.0 更新

- **Agent 主动回复**：Agent 的定时任务、后台脚本或长期工作可以在原聊天请求结束后，通过房间级通道把结果投递回指定 Laphiny 房间，并以绑定的 Agent 身份显示。
- **可靠交付与恢复**：前台使用 SSE 实时接收，另有 15 秒补拉兜底；消息先写入服务端 SQLite 和本地聊天历史，再确认设备游标。事件 ID、单调序号和幂等键共同避免断线、刷新或重试造成丢失与重复。
- **最小权限通道**：设置页可以为具体“房间 × Agent”创建、复制和撤销主动回复配置。令牌不能读取聊天记录、快照或设置，也不能向其他房间或冒充其他成员发送消息。
- **独立自托管后端**：生产同步服务迁移到 [`Lovely-Laper/laphiny-sync`](https://github.com/Lovely-Laper/laphiny-sync)，提供 Docker Compose、自动密钥生成、健康检查和可直接交给服务器 Agent 执行的一句话部署协议。
- **能力边界**：当前不包含 Android/iOS 系统级远程推送；Laphiny 在前台时实时显示，应用重新打开或回到前台后补拉未接收消息。
- **版本统一**：应用内、Expo、npm 与 Android 元数据统一为 `0.35.0`，Android `versionCode` 为 `350`。自动化测试为 132 项。

---

## v0.34.0 更新

- **减负 UI**：连接、房间工具、移动端详情、设置、新手引导和聊天输入区按“常用操作优先、次要选项按需展开”重新分层；所有原功能保留，危险操作与日常操作分离。
- **始终从最新消息开始**：每次重新进入聊天都会定位到最新消息；只有距离最新位置达到 20 条时才显示小型“最新”按钮，避免常驻控件干扰阅读。
- **聊天内委托单**：Agent 发起的每项委托会紧跟来源回复显示任务、委托双方、状态、交付/验收、错误和重试入口，不再依赖隐藏的协作抽屉才能确认委托是否存在。
- **可处理提示**：委托上限、工具委托、记忆草案、目标暂停和 RP 模式提示带有对应快捷动作；设置以聊天内底部弹层呈现，保持滚动位置，保存后立即生效。普通模式每轮委托数成为 1–6 的房间配置，并同步进入 Agent 协作协议与团队模板。
- **真实 Android 验收**：Android 16 模拟器覆盖安装生产签名 Release 后，验证了最新消息定位、委托单、旧提示兼容、快捷设置弹层和即时更新。自动化测试为 124 项。
- **版本统一**：应用内、Expo、npm 与 Android 元数据统一为 `0.34.0`，Android `versionCode` 为 `340`。
...
当前测试覆盖 132 个用例，包括主动消息恢复、SSE 解析、流式事件、权限续跑、委托边界、调度隔离、Goal 状态机、长历史分页和索引修复。
...
要一键重新构建、启动本地生产预览并自动打开浏览器，运行：

```bash
npm run web:preview
```

Windows 也可以直接双击仓库根目录的 `打开Web预览.cmd`。预览默认打开 `http://127.0.0.1:8080/laphiny/`；如果端口被占用，会自动尝试后续端口，并在终端显示实际地址。按 `Ctrl+C` 停止预览。
...
生产同步后端已拆分为独立的 `laphiny-sync` 项目，适合由用户在自己的云服务器上通过 Docker Compose 部署。它保存远端快照，并为每个“房间 × Agent”签发最小权限主动回复通道：Agent 的定时任务即使在原请求结束后完成，也能把结果可靠地投递回对应房间。
...
Laphiny 前台通过 SSE 实时接收主动消息，每 15 秒补拉兜底，并在启动或回到前台时立即恢复。消息先写入服务端 SQLite 和 App 本地历史，再确认游标，因此断网、刷新和重复投递不会丢失或重复显示。
...
App 设置页可以为具体房间成员生成并复制 Agent 投递配置，也可以随时撤销。生产环境必须使用 HTTPS，并妥善保护 `.env` 与 SQLite 数据卷；完整快照可能包含 Hermes API Key。

仓库内的 `npm run sync:server` 只保留为旧接口兼容和本地开发参考，新部署应使用独立项目。
